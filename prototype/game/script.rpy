...
define playername="나"
define m = Character("playername",dynamic=True ,color="#4B89DC")
...
label start:
    ...
    scene bg black with fade 
    $renpy.pause(2.0,hard=True)
    $playername=renpy.input("당신의 이름은?")
    m "내 이름은 [playername].\n{w}나이 23세 컴퓨터 공학부 2학년{w} 몇달 전 군대에서 전역한 복학생이다."
    m "지금은 군대에서 모아둔 월급으로 대학 근처에 있는 원룸에서 자취중이다."
    ...
    m "규칙적인 생활패턴 같은건 이미 초기화된지 오래, 군대가서 사람이 바뀐다는건 개소리다 얼마 못간다."
...
label chapter1:
    ...
    menu:
        "그래 전화 걸어준게 어디야. 누군지는 모르지만 일단 받아 보자.":
            jump chapter1_1_1
        "광고전화나 설문조사겠지. 그냥 무시하자.":
            jump chapter1_1_2
    ...
    label chapter1_1_1:
            ...
            m "그리고 내린 결론은...."
            m "「….? 누구세요?」"
            m "순간적인 고민끝에 내놓은 대답에 잠깐 상대방의 목소리가 끊겼다"
            ...
            m "하지만 그런건 나에게 존재하지 않았다.."
            m "첫 학기 초반에는 나름대로 내 모습을 바꿔보려 시도는 해봤다."
            m "하지만 학창시절때 부터 글러먹은 사교성은 대학교에 와서도 별반 달라지지 않았다."
            m "뭐, 유일하게 애들이 나한테 말 걸때는 수업때 코딩 조금 물어보는정도 밖에 없다."
            m "혼밥은 기본이고, 내 유일한 친구는 노트북이랑 스마트폰밖에 없었는데"
            m "그런데.."
            m "그럼, 이 여잔.. 대체 누구지?"
            m "내 이름이랑 동아리를 아는걸 보면 나를 아는게 맞는거같은데.."
            m "일단 동아리실로 가볼까?"
            m "나는 동아리실로 향했다."
            jump chapter1_1
    ...
    label chapter1_1_2:
            stop sound
            m "뭐 전역해도 연락해주는 친구 하나 없는데 전화 걸어준건 고맙다 임마."
    ...
    label chapter1_1:
            ...
            play music "audio/chap1_walking.mp3" 
            m "언어 마술사 동아리, 내가 이 학교에 입학하고 나서 뜻 맞는 컴퓨터 관련학과 학부생들이 만든 동아리이다."
            m "아, 동아리에 대한 환상도 있었는데 이것도 깨진지 오래다."
            m "보통 동아리가 그렇듯이, 활동은 뒷전이고 술이나 퍼마시는 동아리나 다름없었다."
            m "물론 나는 거기에 끼지 못하고 구석에서 쳐박혀서 코딩이나했지만.."
            m "그런데 전역한 뒤 내 기억과는 전혀 다른 소문을 듣게 되었는데" 
